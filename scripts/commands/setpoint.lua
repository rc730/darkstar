...
local function pointHelp(player)
	player:PrintToPlayer("-------------------------------------------------------------------------------------------", 0x0F);
	player:PrintToPlayer("@setpoint usage:",0x0F);
	player:PrintToPlayer("-------------------------------------------------------------------------------------------", 0x0F);
	player:PrintToPlayer("",0x10);
	player:PrintToPlayer("  Create a new set point:",0x10);
	player:PrintToPlayer("  @setpoint <number> <an optional description>",0x15);
	player:PrintToPlayer("",0x10);
	player:PrintToPlayer("  Goto a set point:",0x10);
	player:PrintToPlayer("  @setpoint goto <number>",0x15);
	player:PrintToPlayer("",0x10);
	player:PrintToPlayer("  View set point descriptions:",0x10);
	player:PrintToPlayer("  @setpoint list",0x15);
	player:PrintToPlayer("",0x10);
	player:PrintToPlayer("  Delete a set point:",0x10);
	player:PrintToPlayer("  @setpoint del <number>",0x15);
	player:PrintToPlayer("",0x10);
	player:PrintToPlayer("  Delete all set points:",0x10);
	player:PrintToPlayer("  @setpoint del all",0x15);
	player:PrintToPlayer("",0x10);
	player:PrintToPlayer("                * <number> must be a value between and including 1 and 9.", 0x0E);
	player:PrintToPlayer("-------------------------------------------------------------------------------------------", 0x0F);
	player:PrintToPlayer("",0x10);
...
local function createPoint(player,pointNumber,description)
	...
	if (player:isInMogHouse()) then
		player:PrintToPlayer("Cannot create a point inside a mog house.",0x0E);
		return;
	end

	if (checkPointNumber(pointNumber) == false) then
		player:PrintToPlayer("Set point '" .. pointNumber .. "' is not a valid set point.",0x0E);
		return;
	end
...
local function delPoint(player,pointNumber)
	...
	if (pointNumber == "all") then
		...
		player:PrintToPlayer("All set points deleted.",0x0F);
	else
		if (checkPointNumber(pointNumber) == false) then
			player:PrintToPlayer("Set point '" .. pointNumber .. "' is not a valid set point.",0x0E);
			return;
		...
		player:PrintToPlayer("Set point " .. pointNumber .. " has been deleted.",0x0F);
	end
end;
...
local function gotoPoint(player,pointNumber)
	...
	if (player:isInMogHouse()) then
		player:PrintToPlayer("The command '@setpoint goto' will not work while in mog house.",0x0E);
		return;
	end
	...
	if (zone ~= 0) then
		...
		player:PrintToPlayer("Set point " .. pointNumber .. " has not been set.",0x0E);
	end
end;
...
local function listPoints(player)
	...
	for pointNumber = 1, 9 do
		...
		if (zone ~= 0) then
			...
			if (description == "") then
				description = getZoneName(zone) .. " (x: " .. xPos .. ", y: " .. yPos .. ", z: " .. zPos .. ")";
			else
				description = description .. " [" .. getZoneName(zone) .. " (x: " .. xPos .. ", y: " .. yPos .. ", z: " .. zPos .. ")]";
			end

			 
		end;
		
		if (pointNumber % 2 == 0) then
			color = 0x15; -- yellow
		else
			color = 0x10; -- blue
		end

		player:PrintToPlayer( pointNumber .. " = " .. description, color);
	end
