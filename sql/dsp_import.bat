...
mysqladmin -h localhost -u root -p@Cer1234 DROP dspdb

ECHO Creating Database dspdb
mysqladmin -h localhost -u root -p@Cer1234 CREATE dspdb

ECHO Loading dspdb tables into the database
cd g:\server\darkstar\sql
FOR %%X IN (*.sql) DO ECHO Importing %%X & "c:\program files\mysql\mysql server 5.6\bin\mysql" dspdb -h localhost -u root -pM@Cer1234 < %%X
